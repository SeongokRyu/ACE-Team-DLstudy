...
class WideResNet():
    ...
    def classifier(self, x, training, **kwargs):
        ...
        def residual(x0, filters, stride=1):
            ...
            x = tf.nn.elu(x)
            x = tf.layers.conv2d(x, filters, 3, 1, **conv_args(3, filters))
            ...
            if x0.get_shape()[3] != filters or x0.get_shape()[1] != x.get_shape()[1]:
                x0 = tf.layers.conv2d(x0, filters, 1, stride, **conv_args(1, filters))
            return x+x0    
        ...
        with tf.variable_scope('classifier', reuse=tf.AUTO_REUSE):
            ...
            for scale in range(len(channels)):
                #y = residual(y, channels[scale], stride=2 if scale else 1)
                y = residual(y, channels[scale], stride=2)
                for i in range(self.repeat-1):
                    y = residual(y, channels[scale])
            y = tf.reduce_mean(y, [1,2]) # Global Average Pooling                                
            logits = tf.layers.dense(y, self.n_classes, kernel_initializer=tf.glorot_normal_initializer())
        return logits            
    ...
    def ops_dict(self, wd, **kwargs):
        ...
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=l_onehot, 
                                                                         logits=logits))
        ...
        train_op = tf.contrib.opt.AdamWOptimizer(
            weight_decay=wd, learning_rate=lr).minimize(loss, decay_var_list=kernels)
